...
void insert_list_m_elements(ListPointer *List, int n){
    ...
    if (EmptyList(*List)){
        ...
        CurrPtr = *List;
        i = 0;
        while( CurrPtr->Next!=NULL && i!=n){
            TempPtr = CurrPtr;
            CurrPtr = CurrPtr->Next;
            i++;
        }
       if(i<n || n<1){ //???????? δεν ξερω αλλά δουλευει
            printf("ERROR\n");
        }else{
            ...
            for(i=0; i<m; i++){
                ...
                scanf("%d", &Item); //Στην 1η επανάληψη το CurrPtr (τρέχον στοιχείο) είναι το στοιχείο στη θέση n ΣΛ (από τον προηγούμενο βρόχο)
